...
const questions = [
...

    {
        ...
        explanation: "emang bisa nyalain yang lain selain korek dulu? xixixi"
    },
...

    {
        ...
        explanation: "Semua simbol itu ada (kota, gunung, sungai), tapi secara fisik tidak ada di sana. "
    }
...
function endGame() {
    ...
    document.getElementById('score').innerText = score; // Tampilan di layar akhir
    
    // BAGIAN PENTING: Mengirim ke Firebase
    const lbRef = window.dbRef(window.db, 'leaderboard');
    
    // Pastikan 'score' di sini adalah variabel yang nilainya sudah ribuan
    window.dbPush(lbRef, { 
        name: currentPlayerName, 
        score: Number(score) // Pakai Number() untuk memastikan yang dikirim adalah angka, bukan teks
    });
}
